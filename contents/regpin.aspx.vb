...
Partial Class regpin
    ...
    Protected Sub cmd_save_Click(sender As Object, e As EventArgs) Handles cmd_save.Click
        ' If validar_dlist(dl_medios) = True Then
        ' If validar_dlist(dl_razon) = True Then
        guardaryenviar()
        '     Else
        'mv_msgs.ActiveViewIndex = 17
        'hf_popupok_mpe.Show()
        'End If
        'Else
        ' mv_msgs.ActiveViewIndex = 17
    ...
    Private Function validar_dlist(ByVal dl As DataList) As Boolean
        Dim t As Integer = 0
        Dim cnt As Integer = 0
        For t = 0 To dl.Items.Count - 1
            Dim cbx As CheckBox = CType(dl.Items(t).FindControl("cbx_select"), CheckBox)
            If cbx.Checked = True Then
                cnt = cnt + 1
            End If
        Next
        Return IIf(cnt > 0, 1, 0)
    End Function
End Class
